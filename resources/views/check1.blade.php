...
<body>
  <div class="app">
    ...
    <main class="content">
      ...
      <div class="doc-row">
        <span style="color:var(--muted);font-size:13px;">Hujjat:</span>
        <span class="doc-pill">{{ $paymentCheck->document_number }}</span>
        <span class="status-inline">{{ $paymentCheck->status_label }}</span>
      </div>
      ...
      <section class="block">
        <h2 class="block-title">Summa</h2>
        @if(filled($paymentCheck->amount_hint))
          <div class="label">{{ $paymentCheck->amount_hint }}</div>
        @endif
        <div class="value-box" style="margin-top:10px;border-color:rgba(22,216,255,.35);">
          <div class="value-text" style="font-size:22px;font-weight:800;">{{ $paymentCheck->amount_display }}</div>
        </div>
      </section>
